...
public class DetailFrag2 extends Fragment {
    ...
    class ProfileThread extends Thread {
        ...
        public void run() {
            ...
            try {
                ...
                String data = "column=" + column + "&tableNum=" + tableNum;
                OutputStream os = connection.getOutputStream();
                os.write(data.getBytes());
    ...
    private void inflateProfiles() {
        if (profileBeens.size() > 6) {
            for (int i = 0; i < 6; i++) {
                ...
                imageView.setOnClickListener(onProfileClickListener(i));
            ...
            if (G.isLogin == true) { //로그인된 회원
                ...
                new AlertDialog.Builder(getActivity()).setMessage("로그인이 필요합니다. 로그인 하시겠습니까?").setPositiveButton("로그인", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        Intent intent = new Intent(getActivity(), LoginActivity.class);
                        startActivity(intent);
                        getActivity().overridePendingTransition(R.anim.anim_slide_in_bottom, R.anim.anim_transparent);
                        getActivity().finish();
                    }
                }).setNegativeButton("다음에", null).create().show();
    ...
    public void setListViewHeightBasedOnChildren(ListView listView) {
        ...
        if (reviewBeen.size() >= 5) {

            for (int i = 0; i < 4; i++) {//리뷰가 5개가 넘어가면 리뷰 5개만 보이게 하기
