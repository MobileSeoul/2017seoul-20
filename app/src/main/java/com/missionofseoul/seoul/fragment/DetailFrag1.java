...
public class DetailFrag1 extends Fragment {
    ...
    TextView reviewTotalNum;
    TextView clicktotal_review;
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        thumImgContainer = (LinearLayout) view.findViewById(R.id.layout_linear);
        ...
        listView_review.setOnItemLongClickListener(new AdapterView.OnItemLongClickListener() {
            ...
            public boolean onItemLongClick(AdapterView<?> adapterView, View view, int position, long l) {
                ...
                if (G.mem_id.equals(reviewBeen.get(position).getId())) {//아이디가 같으면
                    Toast.makeText(getContext(), "리뷰 삭제하실 수 없습니다.", Toast.LENGTH_SHORT).show();
    ...
    //서버에서 해당 미션수행한사진 가져오기 //썸네일로 보여주는 이미지 (비회원/회원 동일하게 보인다)
    class ThumGalleryThread extends Thread {
        ...
        public void run() {
            try {
                ...
                getActivity().runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        setListViewHeightBasedOnChildren(listView_review);
    ...
    View.OnClickListener onProfileClickListener(final int position) {
        return new View.OnClickListener() {
            @Override
            public void onClick(View view) {
